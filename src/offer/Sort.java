...
public class Sort {
    ...
    private static void recQuickSort(int[] array,int left,int right){
        if(right <= left){
            ...
            int partition = partitionIt(array,left,right);// 确定出基准元素归位的下标
    ...
    private static int partitionIt(int[] array,int left,int right){
        ...
        while(true){
            while(i<right && array[++i] < pivot){} // 确定出比基准元素大或相等的下标i
            while(j > 0 && array[--j] > pivot){}   // 确定出比基准元素小或相等的下标j
